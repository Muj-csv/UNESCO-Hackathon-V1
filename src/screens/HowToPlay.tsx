...
/* ============================================================================
   ...
   Reached from the lobby on request, never on the way into a round — a room
   that already knows the game should not have to read it again.

   Four beats, under ninety seconds: what happens, the five things worth
   keeping, the one rule, and what the end of a round looks like. The atoms are
   the vocabulary a player takes away from the whole session, so they are named
   here before anyone plays rather than explained afterwards in the ledger.
   ========================================================================== */
...
const ATOM_NOTES: { atom: Atom; holds: string; dies: string }[] = [
  { atom: 'SOURCE', holds: 'Who says so.', dies: '“studies show”, with nobody named' },
  { atom: 'NUMBER', holds: 'The figure, and what it was measured against.', dies: 'a percentage with its base dropped' },
  { atom: 'HEDGE', holds: 'May. Suggests. Preliminary.', dies: 'an early finding retold as settled' },
  { atom: 'SCOPE', holds: 'Who, where, and when.', dies: 'one campus becomes everywhere' },
  { atom: 'CAUSE', holds: 'Whether one thing actually caused the other.', dies: '“linked to” becomes “causes”' },
];
...
export default function HowToPlay() {
  ...
  return (
    <div className="screen">
      ...
      {/* The one rule gets a screen's worth of space and nothing to compete
          with. Everything else in this game follows from believing it. */}
      <div className="paper howto-rule-card">
      ...
      <div className="card">
        <p className="eyebrow">What you get at the end</p>
        <p className="muted">
          The Decay Ledger, one row per property. Not a score — a record of what was lost, where,
          and what pressure took it.
        </p>
